...
public class AvatarController {
    ...
    @PostMapping
    public ResponseEntity<?> addAndSetNewCustomAvatar(@Auth Long id, @Valid @RequestBody AvatarUrlDto avatarDto) {
        avatarService.addAndSetNewCustomAvatar(id, avatarDto.avatarUrl());
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @GetMapping
    public ResponseEntity<?> getAvatars(@Auth Long id) {
        return ResponseEntity.ok(avatarService.getAvatars(id));
    }

    @PatchMapping("/{avatarId}")
    public ResponseEntity<?> chooseExistingCustomAvatar(@Auth Long id, @PathVariable Long avatarId) {
        avatarService.chooseExistingCustomAvatar(id, avatarId);
        return ResponseEntity.ok().build();
    }

    @PatchMapping("/default")
    public ResponseEntity<?> chooseDefaultAvatar(@Auth Long id, @Valid @RequestBody AvatarUrlDto avatarDto) {
        avatarService.chooseDefaultAvatar(id, avatarDto.avatarUrl());
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/{avatarId}")
    public ResponseEntity<?> deleteCustomAvatar(@Auth Long id, @PathVariable Long avatarId) {
        avatarService.deleteCustomAvatar(id, avatarId);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }

    @PatchMapping("/current")
    public ResponseEntity<?> resetCurrentAvatar(@Auth Long id) {
