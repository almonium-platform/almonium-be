...
public interface FriendshipRepository extends JpaRepository<Friendship, Long> {
     ...
     *
     * <p>This method performs a query on the User and Friendship tables, joining them based on the user's ID.
     * It returns a list of FriendInfoView objects, which are projections that include the user's ID, the status of the friendship,
     * and a boolean indicating whether the user is the requester of the friendship.</p>
     *
     * <p>The query filters out friendships where the current user has been blocked by the other user or the status is MUTUALLY_BLOCKED.
     * Specifically, it excludes records where:</p>
     * <ul>
     ...
     * @return A list of FriendInfoView objects representing the visible friendships for the given user.
     */
    @Query(
            """
            ...
            from User u
            join Friendship f on (u.id = f.requester.id or u.id = f.requestee.id)
            where u.id = :id
            and not ((f.requester.id = :id and str(f.status) = 'SND_BLOCKED_FST')
             or (f.requestee.id = :id and str(f.status) = 'FST_BLOCKED_SND'))
            """)
    List<FriendInfoView> getVisibleFriendships(@Param("id") long id);
}
